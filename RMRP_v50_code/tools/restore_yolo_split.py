...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        choices=["rcadnet", "rmrp_fusion", "dfpir", "nafnet", "nafnet_meta", "demoe", "instructir"],
        required=True,
    )
    ...
    parser.add_argument("--rmrp-motion-threshold", type=float, default=0.18)
    parser.add_argument("--rmrp-defocus-threshold", type=float, default=0.20)
    parser.add_argument("--rmrp-lowlight-threshold", type=float, default=0.385)
    parser.add_argument("--rmrp-expert-support-threshold", type=float, default=0.50)
    parser.add_argument("--rmrp-lowlight-dfpir-weight", type=float, default=0.40)
    parser.add_argument("--rmrp-mixed-dfpir-weight", type=float, default=0.075)
    parser.add_argument("--rmrp-sensor-gyro-full-scale", type=float, default=4.0)
    parser.add_argument(
...
def main() -> None:
    ...
    if args.model == "rmrp_fusion":
        ...
        if missing:
            raise ValueError(
                "RMR-P expert fusion requires " + ", ".join(missing)
            )
        policy = ExpertFusionPolicy(
            motion_threshold=args.rmrp_motion_threshold,
            defocus_threshold=args.rmrp_defocus_threshold,
            lowlight_threshold=args.rmrp_lowlight_threshold,
            support_threshold=args.rmrp_expert_support_threshold,
            lowlight_dfpir_weight=args.rmrp_lowlight_dfpir_weight,
            mixed_dfpir_weight=args.rmrp_mixed_dfpir_weight,
            gyro_full_scale=args.rmrp_sensor_gyro_full_scale,
        )
        model = RMRPExpertFusion(
            DeMoEAdapter(
                args.demoe_weights,
                device=device,
                task="auto",
    ...
    with torch.inference_mode():
        for index, path in enumerate(paths):
            ...
            tensor, original_size = pad_to_multiple(
                tensor,
                multiple=16 if args.model == "instructir" else (1 if args.model == "rmrp_fusion" else 8),
            )
            if device.type == "cuda":
            ...
            if args.model == "rmrp_fusion":
                ...
                if args.rcadnet_metadata_control == "unavailable":
                    ...
                    if args.rcadnet_metadata_control == "zero":
                        ...
                        raise ValueError(
                            "RMR-P expert fusion supports correct, zero, unavailable, "
    ...
    runtime = {
        ...
        "rmrp_expert_fusion_policy": (
            ...
            if isinstance(model, RMRPExpertFusion)
            else None
        ),
        "rmrp_expert_fusion_route_counts": (
            {
