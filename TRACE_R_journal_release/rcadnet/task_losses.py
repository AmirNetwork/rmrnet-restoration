...
def unwrap_detector(detector: nn.Module) -> nn.Module:
    ...
    """
    # Ultralytics ``DetectionModel`` is already the executable graph. Its
    # ``.model`` attribute is only a sequential module list and cannot be run
    # directly because skip/concat routing is implemented by DetectionModel.
    # Keep that object intact; only unwrap genuine outer wrappers.
    module_name = detector.__class__.__module__
    if module_name.startswith("ultralytics.nn.tasks"):
        return detector
...
class FrozenDetectorSupervisedLoss(nn.Module):
    """Optimize restored training crops against frozen YOLO box/class targets.

    Unlike feature matching alone, this term uses the detector's actual
    supervised objective while keeping every detector parameter frozen:

        L_FDS = log(1 + L_YOLO(Phi(I_mix), Y) / s_c)
              + beta log(1 + [L_mix - L_clean + margin]_+ / s_c),

        s_c = max(stopgrad(L_YOLO(Phi(I_c), Y)), s_min).

    where ``Y`` contains boxes transformed by the dataset's exact crop and
    flip, and ``I_mix`` is the same-geometry restored/clean CQMix image. The
    clean-relative normalization makes the coefficient portable across
    detectors and batch sizes. Fidelity and TDP terms remain active in the same
    backward pass to prevent detector-only adversarial shortcuts.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Ultralytics checkpoints loaded outside a Trainer can retain a plain
        # dict in model.args. Its detection criterion requires the complete
        # namespace containing box/cls/DFL gains.
        args = getattr(self.detector, "args", None)
        if isinstance(args, dict):
            try:
                from ultralytics.cfg import DEFAULT_CFG_DICT, get_cfg

                self.detector.args = get_cfg(DEFAULT_CFG_DICT, overrides=args)
            except Exception as exc:
                raise RuntimeError(
    ...
    def _flatten_targets(
        ...
        if classes.ndim != 2 or valid.shape != classes.shape:
            raise ValueError(
                "detector classes and valid mask must both have shape BxN"
            )
        if bboxes.shape[:2] != classes.shape or bboxes.shape[-1] != 4:
            raise ValueError("detector boxes must have shape BxNx4")

        valid = valid.to(dtype=torch.bool)
        batch_index = torch.arange(
            classes.shape[0],
            device=classes.device,
        )[:, None].expand_as(classes)
        return {
            "cls": classes[valid].to(dtype=bboxes.dtype),
            "bboxes": bboxes[valid].to(dtype=bboxes.dtype).clamp(0.0, 1.0),
            "batch_idx": batch_index[valid].to(dtype=bboxes.dtype),
        }
    ...
    def _detector_loss(
        ...
        result = self.detector.loss(batch)
    ...
    def forward(
        ...
        *,
        ...
        # unbounded ratio and lets a single crop dominate the restorer update.
        # A detector-scale floor plus log compression retains the ranking and
        # gradient direction while making the task term robust.
        denominator = clean_total.detach().abs().clamp_min(
            max(self.normalization_floor, self.eps)
        )
        normalized = restored_total / denominator
        hinge = F.relu(
            restored_total
            - clean_total.detach()
            + self.clean_margin,
        ) / denominator
        loss = torch.log1p(normalized.clamp_min(0.0))
        loss = loss + self.clean_hinge_weight * torch.log1p(hinge)
        self.last_stats = {
            "detector_supervised_restored": restored_total.detach(),
            "detector_supervised_clean": clean_total.detach(),
            "detector_supervised_ratio": normalized.detach(),
            "detector_supervised_normalization_scale": denominator.detach(),
            "detector_supervised_hinge": hinge.detach(),
            "detector_supervised_targets": restored.new_tensor(
                float(targets["cls"].numel())
            ),
            "detector_supervised_box": restored_parts[0].detach()
            if restored_parts.numel() > 0
            else restored.new_zeros(()),
            "detector_supervised_cls": restored_parts[1].detach()
            if restored_parts.numel() > 1
            else restored.new_zeros(()),
            "detector_supervised_dfl": restored_parts[2].detach()
            if restored_parts.numel() > 2
            else restored.new_zeros(()),
            "detector_supervised_clean_box": clean_parts[0].detach()
            if clean_parts.numel() > 0
            else restored.new_zeros(()),
            "detector_supervised_clean_cls": clean_parts[1].detach()
            if clean_parts.numel() > 1
            else restored.new_zeros(()),
            "detector_supervised_clean_dfl": clean_parts[2].detach()
            if clean_parts.numel() > 2
            else restored.new_zeros(()),
        }
        return loss
...
def hutchinson_jacobian_penalty(
    ...
    *,
    ...
    # Detector Jacobians require higher-order gradients. FP16 autocast is
    # numerically unsafe for this derivative even when the scalar penalty is
    # finite, so keep this branch in FP32 while allowing the rest of RMR-Net
    # training to use AMP. ``restored.float()`` remains differentiably linked
    # to the restoration output.
    with torch.amp.autocast(device_type=restored.device.type, enabled=False):
        x = restored.float()
        if not x.requires_grad:
            x = restored.detach().float().requires_grad_(True)
        else:
        ...
        for _ in range(int(num_probes)):
            ...
            projection = x.new_tensor(0.0)
            for feat in features.values():
                v = torch.empty_like(feat).bernoulli_(0.5).mul_(2.0).sub_(1.0)
                projection = projection + (feat * v).sum() / float(
                    max(feat.numel(), 1) ** 0.5
                )
...
class CompositeTaskLoss(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        detector_supervised_loss: Optional[FrozenDetectorSupervisedLoss] = None,
        ...
        self.detector_supervised_loss = detector_supervised_loss
    ...
    def forward(
        ...
        *,
        ...
        detector_targets: Optional[Dict[str, torch.Tensor]] = None,
        ...
        if self.weights.detector_supervised > 0:
            if self.detector_supervised_loss is None:
                raise RuntimeError(
                    "FrozenDetectorSupervisedLoss is required because "
                    "weights.detector_supervised > 0."
                )
            if detector_targets is None:
                raise RuntimeError(
                    "Training detector targets are required because "
                    "weights.detector_supervised > 0."
                )
            loss_detector = self.detector_supervised_loss(
                restored,
                clean,
                classes=detector_targets["classes"],
                bboxes=detector_targets["bboxes"],
                valid=detector_targets["valid"],
            )
            weighted = (
                scale * self.weights.detector_supervised * loss_detector
            )
            total = total + weighted
            self._log_scalar(
                logs,
                "loss_detector_supervised_raw",
                loss_detector,
            )
            self._log_scalar(
                logs,
                "loss_detector_supervised_weighted",
                weighted,
            )
            for name, value in self.detector_supervised_loss.last_stats.items():
                self._log_scalar(logs, name, value)
