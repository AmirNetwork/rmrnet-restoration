#!/usr/bin/env python3
"""Build auditable TRACE-R panels from retained validation outputs.

The script scans low-light and mixed validation outputs with the same frozen
detector used for the paper metrics. It selects examples where TRACE-R ties the
best displayed restorer and recovers more class-correct ground-truth instances
than the degraded input. Exact expert routes can make TRACE-R and one constituent
...
def build_figure(
    ...
    # Pavement crops are wide and shallow.  A compact row height prevents large
    # blank bands when the figure is embedded in a two-column journal page.
    figure_height = 0.95 * len(selected) + 0.45
    fig, axes = plt.subplots(
        len(selected), len(METHODS), figsize=(12.5, figure_height), dpi=220
    )
    if len(selected) == 1:
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "selection_rule": "TRACE-R ties the best displayed restorer and recovers more class-correct IoU50 ground truth than degraded input; DeMoE gain, confidence, and GT-count tie-breaks",
    ...
    for dataset in DATASETS:
        ...
        if len(strict) < 2:
            raise RuntimeError(
                f"Only {len(strict)} honest TRACE-R recovery examples found for {dataset}; "
