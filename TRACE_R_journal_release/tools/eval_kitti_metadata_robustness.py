...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-root", default="data/kitti_oxts_physical_test")
    parser.add_argument("--scenario", default="kitti_oxts_physical_motion")
    parser.add_argument("--full-weights", required=True, help="Validation-selected full-prior checkpoint.")
    parser.add_argument("--raw-weights", required=True, help="Validation-selected raw-OXTS checkpoint.")
    parser.add_argument("--image-weights", required=True, help="Validation-selected image-only checkpoint.")
...
def save_qualitative(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    draw.text((pad, 16), "KITTI held-out aligned-telemetry samples (drive 0005)", fill=(20, 32, 42), font=font(18))
    ...
    for row_idx, item in enumerate(selected):
        ...
        draw.text((pad, y + 50), item["name"].replace("2011_09_26_drive_0005_", ""), fill=(20, 32, 42), font=font(12))
...
def main() -> None:
    ...
    full_weights = (ROOT / args.full_weights).resolve() if not Path(args.full_weights).is_absolute() else Path(args.full_weights)
    raw_weights = (ROOT / args.raw_weights).resolve() if not Path(args.raw_weights).is_absolute() else Path(args.raw_weights)
    image_weights = (ROOT / args.image_weights).resolve() if not Path(args.image_weights).is_absolute() else Path(args.image_weights)
    full_model = load_rcadnet(str(full_weights), device)
    raw_model = load_rcadnet(str(raw_weights), device)
    image_model = load_rcadnet(str(image_weights), device)
    ...
    for index, input_path in enumerate(image_paths):
        ...
        blind = unpad(image_model(image_pad.to(device), None).cpu(), original_size)
        outputs["blind"] = blind
        metrics["blind"] = psnr_ssim(blind, target)
        for mode, metadata in full_modes.items():
            code = code_from_metadata(metadata, device=device)
            output = unpad(full_model(image_pad.to(device), code).cpu(), original_size)
            outputs[mode] = output
            metrics[mode] = psnr_ssim(output, target)
        raw_metadata = metadata_for_mode(metadata_records[index], "raw_telemetry")
        raw_code = code_from_metadata(raw_metadata, device=device)
        raw_output = unpad(raw_model(image_pad.to(device), raw_code).cpu(), original_size)
        outputs["raw_telemetry"] = raw_output
        metrics["raw_telemetry"] = psnr_ssim(raw_output, target)
        raw_blind = unpad(raw_model(image_pad.to(device), None).cpu(), original_size)
        outputs["raw_model_blind"] = raw_blind
        metrics["raw_model_blind"] = psnr_ssim(raw_blind, target)
        ...
        row["raw_telemetry_minus_raw_model_blind_psnr"] = row["raw_telemetry_psnr"] - row["raw_model_blind_psnr"]
    ...
    summary = {
        "images": len(rows),
        "no_leakage_check": {
            "train_sequences": ["2011_09_26_drive_0001_sync", "2011_09_26_drive_0002_sync"],
            "validation_sequences": ["2011_09_26_drive_0011_sync"],
        ...
        "median_gain_psnr": {
            ...
            "raw_telemetry_minus_raw_model_blind": median("raw_telemetry_minus_raw_model_blind_psnr"),
        ...
        "fraction_frames_positive_psnr_gain": {
            ...
            "raw_telemetry_over_raw_model_blind": float(np.mean([row["raw_telemetry_minus_raw_model_blind_psnr"] > 0 for row in rows])),
        ...
        "checkpoints": {
            "full_prior": {"path": str(full_weights.relative_to(ROOT)), "sha256": sha256(full_weights)},
            "raw_telemetry": {"path": str(raw_weights.relative_to(ROOT)), "sha256": sha256(raw_weights)},
            "image_only": {"path": str(image_weights.relative_to(ROOT)), "sha256": sha256(image_weights)},
        },
    }
