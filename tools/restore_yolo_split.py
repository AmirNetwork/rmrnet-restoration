...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", choices=["rcadnet", "dfpir", "nafnet", "demoe"], required=True)
...
def load_rcadnet(weights: str, device: torch.device) -> RCADNet:
    ...
    if hasattr(model, "load_pretrained"):
        ...
        if load_report["missing_keys"] or load_report["unexpected_keys"] or load_report["skipped_shape"] or load_report["skipped_missing"]:
            print(
                json.dumps(
                    {
                        ...
                        "policy": "shape-compatible transfer; revised modules keep initialized weights",
                    }
                ),
                flush=True,
            )
