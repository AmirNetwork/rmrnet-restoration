...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "all annotated Sony cam1 frames without resizing, evaluates the "
            "YOLO crop/mask plus smart-tile detector "
    ...
    parser.add_argument(
        "--detector",
        type=Path,
        default=Path("weights/yolo26s_rdd_frdc_distilled_v2/YOLO26s_RDD_FRDC_Distilled_v2.pt"),
    )
    parser.add_argument(
        "--rcadnet-weights",
        type=Path,
        default=Path("runs/rmrnet_v31_gt49_focus_pcm4ep/rcadnet_best.pth"),
        help="RMR-Net checkpoint used by tools/restore_native_yolo_split.py.",
    )
    parser.add_argument(
        "--out-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional existing restored/native_real directory to evaluate. "
            "Use this for detector audits so restored images are not duplicated."
        ),
    )
    parser.add_argument(
        "--models",
        default="raw,rmr_blind,rmr_metadata,rmr_metadata_gated,nafnet,demoe_auto,demoe_scenario",
        help=(
    ...
    parser.add_argument("--eval-strategy", choices=["tiled", "smart_tile", "hybrid"], default="hybrid")
    parser.add_argument("--eval-tile", type=int, default=2048)
    parser.add_argument("--eval-overlap", type=int, default=640)
    parser.add_argument("--center-margin", type=int, default=256)
    parser.add_argument("--infer-imgsz", type=int, default=1536)
    parser.add_argument("--conf", type=float, default=0.5)
    parser.add_argument("--branch-iou", type=float, default=0.78)
    parser.add_argument("--full-imgsz", type=int, default=1536)
    parser.add_argument("--full-conf", type=float, default=0.5)
    parser.add_argument("--full-iou", type=float, default=0.78)
    parser.add_argument("--tile-fusion-conf", type=float, default=0.5)
    parser.add_argument("--nms-iou", type=float, default=0.50)
    parser.add_argument("--crop-bottom-half", action="store_true", default=True)
    parser.add_argument("--skip-restore", action="store_true")
    parser.add_argument("--skip-eval", action="store_true")
    parser.add_argument("--no-overlays", action="store_true")
    parser.add_argument("--no-paper-copy", action="store_true", help="Write experiment outputs but do not overwrite paper assets.")
...
def ensure_eval(args: argparse.Namespace, data_yaml: Path, out_dir: Path) -> None:
    ...
    if args.crop_bottom_half:
        cmd.append("--crop-bottom-half")
    cmd.extend(["--road-filter", "--crack-classes", "0,1,2"])
...
def summarize_eval(eval_dir: Path, model: str, restore_summary: Path) -> dict[str, Any]:
    ...
    row = {
        ...
        "metadata": (
            "gated"
            if model == "rmr_metadata_gated"
            else ("yes" if model in {"rmr_metadata", "instructir_metadata"} else ("scenario" if model == "demoe_scenario" else "no"))
        ),
...
def make_failure_figure(raw_eval: Path, rmr_eval: Path, out_path: Path, limit: int = 4) -> None:
    raw_overlays = sorted((raw_eval / "overlays").glob("*_native_*.png"))
    rmr_by_name = {p.name: p for p in (rmr_eval / "overlays").glob("*_native_*.png")}
...
def main() -> None:
    ...
    restored_root = args.restored_root if args.restored_root is not None else out_root / "restored" / "native_real"
    eval_root = out_root / "native_tiled_eval"
    paper_dirs = [ROOT / "paper_ieee_tits_rmrnet", ROOT / "paper_trc_rmrnet"]
    out_root.mkdir(parents=True, exist_ok=True)

    if args.restored_root is None:
    ...
    make_failure_figure(
        eval_root / "raw",
        eval_root / "rmr_metadata_gated",
        out_root / "figures" / "fig_native_failure_modes.png",
    )

    # Copy final figure assets into the paper project unless this is a detector
    # audit. This keeps manuscript tables tied to the explicitly selected run.
    for paper_dir in ([] if args.no_paper_copy else paper_dirs):
        ...
        (
            ...
            "Detection uses the GT49 crop/mask detector protocol: lower-road native crop, "
            "full-crop inference for road context, 2048-pixel center-safe tiles for local detail, "
            "class-aware fusion NMS, and a fixed road-surface gate."
        ),
        ...
        ["Method", "N", "P@.50", "R@.50", "F1@.50", "P@.10", "R@.10", "F1@.10", "FP/img", "s/img"],
        main_rows,
        )
        write_tex_table(
        ...
        (
            ...
            "All rows use the GT49 crop/mask full-crop plus smart-tile detector."
        ),
        ...
        ["Policy", "P@.50", "R@.50", "F1@.50", "P@.10", "R@.10", "F1@.10", "FP/img"],
        eta_rows,
        )
        write_tex_table(
        ...
        (
            ...
            "a frame to the restorer only when the score gain exceeds $\\tau$; detection uses "
            "the same GT49 crop/mask detector protocol for every policy."
        ),
        ...
        ["Policy", "Raw", "Restored", "P@.50", "R@.50", "F1@.50", "P@.10", "R@.10", "F1@.10", "FP/img"],
        tau_rows,
        )
        write_tex_table(
    ...
    manifest = {
        ...
        "detector_strategy": args.eval_strategy,
        "hybrid_settings": {
            ...
            "tile_imgsz": args.infer_imgsz,
            "tile_conf": args.conf,
            "tile_iou": args.branch_iou,
