...
# Paper role: reproducible qualitative rendering of the frozen-detector results.
...
def variant_dirs(config: AtlasConfig, tag: str) -> dict[str, Path]:
    return {
        ...
        "DeMoE-scenario": ROOT / "datasets" / f"{config.prefix}_{tag}_test_demoe_scenario" / "images" / "test",
...
def select_candidates(config: AtlasConfig, scenarios: list[str], rows_per_scenario: int, max_candidates: int, conf: float) -> list[dict]:
    ...
    for tag in scenarios:
        ...
        for name in names:
            ...
            rmr_margin = matches["RMR+meta"] - max(
                matches["NAFNet"], matches["DFPIR"], matches["DeMoE-scenario"]
            )
        ...
        # A qualitative success panel must actually support the stated claim.
        # Selection is deterministic and its complete score ledger is released.
        positive = [row for row in scored if row["rmr_gain"] > 0 and row["rmr_margin"] >= 0]
        pool = positive if len(positive) >= rows_per_scenario else scored
        pool.sort(key=lambda row: row["score"], reverse=True)
        selected.extend(pool[:rows_per_scenario])
    return selected
...
def build_atlas(config: AtlasConfig, candidates: list[dict], out_name: str, conf: float, *, zoom: bool = False) -> Path:
    ...
    model = YOLO(str(config.detector))
    columns = ["Clean", "Degraded", "NAFNet", "DFPIR", "DeMoE-scenario", "RMR+meta"]
    tile = (250, 142)
    gap = 10
    left = 168
    top = 102
...
def main() -> None:
    ...
    configs = [
        AtlasConfig(
            ...
            detector=ROOT / "runs" / "detect" / "runs" / "yolo11s_v26" / "pcm_clean_80ep" / "weights" / "best.pt",
        ...
        AtlasConfig(
            name="IVCNZ pothole",
            detector=ROOT / "runs" / "detect" / "runs" / "yolo11s_v26" / "pothole_clean_80ep" / "weights" / "best.pt",
